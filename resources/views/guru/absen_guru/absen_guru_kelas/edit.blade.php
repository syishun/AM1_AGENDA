        ...
        <div class="w-full md:w-1/2 mb-4 md:mb-0">
            <form action="{{ url('absen_guru/' . $absen_guru->id) }}" method="post" enctype="multipart/form-data" class="space-y-4">
                ...
                <div class="form-group">
                    ...
                    <input type="date" class="mt-1 block w-full h-10 bg-gray-200 rounded-md shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm @error('tgl') border-red-500 @enderror" id="tgl" name="tgl" value="{{ old('tgl', $absen_guru->tgl) }}" style="padding-left: 10px;">
                    @error('tgl')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <label class="flex w-full cursor-pointer" style="margin: 10px; margin-left: 0;">
                        ...
                        <input type="file" class="hidden" id="tugas" name="tugas" value="{{ old('tugas', $absen_guru->tugas) }}">
                    </label>
                    @error('tugas')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
