<x-layout>
    ...
    <div class="flex flex-col md:flex-row justify-between items-center mb-6 p-4 space-y-4 md:space-y-0 md:space-x-4">
        <form method="GET" action="{{ route('absen_siswa.admin_index') }}" class="flex flex-col md:flex-row items-center w-full md:w-auto space-y-4 md:space-y-0 md:space-x-2">
            <input type="text" name="search" placeholder="Cari siswa atau kelas..." value="{{ $search ?? '' }}" class="border rounded p-2 w-full md:w-64 focus:outline-none focus:ring-2 focus:ring-green-500 transition duration-200">
            ...
            <div class="flex space-x-2 w-full md:w-auto">
                ...
                <a href="{{ route('absen_siswa.admin_index') }}" class="bg-gray-500 text-white py-2 px-4 rounded hover:bg-gray-600 transition duration-200 w-full md:w-auto text-center">Reset</a>
            </div>
        </form>
    </div>
