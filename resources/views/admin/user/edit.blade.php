        ...
        <div class="w-full md:w-1/2 mb-4 md:mb-0">
            <form action="{{ url('user/' . $user->id) }}" method="post" enctype="multipart/form-data" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" class="mt-1 block w-full h-10 bg-gray-200 rounded-md shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm @error('name') border-red-500 @enderror" id="name" name="name" value="{{ old('name', $user->name) }}" style="padding-left: 10px;">
                    @error('name')
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="password" class="mt-1 block w-full h-10 bg-gray-200 rounded-md shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm @error('password') border-red-500 @enderror" id="password" name="password" value="{{ old('password') }}" placeholder="Kosongkan jika tidak ingin mengganti" style="padding-left: 10px;">
                ...
                <div>
                    ...
                    <div class="mt-2 flex items-center space-x-4">
                        ...
                        <div>
                            <input type="radio" id="role_perwakilan_kelas" name="role" value="Perwakilan Kelas" {{ old('role', $user->role) == 'Perwakilan Kelas' ? 'checked' : '' }} class="text-green-500 focus:ring-green-500">
                ...
                <div id="kelas-section" style="display: none;">
                    ...
                    <select class="mt-1 block w-full h-10 bg-gray-200 rounded-md shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm @error('kelas_id') border-red-500 @enderror" name="kelas_id" id="kelas_id" style="padding-left: 10px;">
                        ...
                        @foreach ($kelas as $item)
                            <option value="{{ $item->id }}" {{ old('kelas_id', $user->kelas_id) == $item->id ? 'selected' : '' }}>{{ $item->kelas_id }}</option>
                        @endforeach
                    </select>
                    @error('kelas_id')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
