<x-layout>
    ...
    <div class="container mx-auto p-6 flex flex-col md:flex-row">
        ...
        <div class="w-full md:w-1/2 mb-4 md:mb-0">
            <form action="{{ url('jurusan') }}" method="post" enctype="multipart/form-data" class="space-y-4">
                @csrf

                <div>
                    <label for="jurusan_id" class="block text-sm font-medium text-gray-700">Nama Jurusan</label>
                    <input type="text" class="mt-1 block w-full h-10 bg-gray-200 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-green-500 sm:text-sm @error('jurusan_id') border-red-500 @enderror" id="jurusan_id" name="jurusan_id" value="{{ old('jurusan_id') }}" style="padding-left: 10px;">
                    @error('jurusan_id')
