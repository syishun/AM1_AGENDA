        ...
        <div class="w-full md:w-1/2 mb-4 md:mb-0">
            <form action="{{ url('data_siswa/' . $data_siswa->id) }}" method="post" enctype="multipart/form-data" class="space-y-4">
                ...
                <div>
                    ...
                    <select class="mt-1 block w-full h-10 bg-gray-200 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-green-500 sm:text-sm @error('kelas_id') border-red-500 @enderror" name="kelas_id" id="kelas_id" style="padding-left: 10px;">
                        @foreach ($kelas as $item)
                            <option value="{{ $item->id }}" {{ $data_siswa->kelas_id == $item->id ? 'selected' : '' }}>{{ $item->kelas }} {{ $item->jurusan->jurusan_id }} {{ $item->kelas_id }} ({{ $item->thn_ajaran }})</option>
                        @endforeach
                    </select>
                    @error('kelas_id')
                ...
                <div>
                    ...
                    <input type="number" class="mt-1 block w-full h-10 bg-gray-200 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-green-500 sm:text-sm @error('nis_id') border-red-500 @enderror" id="nis_id" name="nis_id" value="{{ old('nis_id', $data_siswa->nis_id) }}" style="padding-left: 10px;">
                    @error('nis_id')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
