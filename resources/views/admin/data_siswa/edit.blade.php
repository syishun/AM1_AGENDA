        ...
        <div class="w-full md:w-1/2 mb-4 md:mb-0">
            <form action="{{ url('data_siswa/' . $data_siswa->id) }}" method="post" enctype="multipart/form-data" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" class="mt-1 block w-full h-10 bg-gray-200 rounded-md shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm @error('nama_siswa') border-red-500 @enderror" id="nama_siswa" name="nama_siswa" value="{{ old('nama_siswa', $data_siswa->nama_siswa) }}" style="padding-left: 10px;">
                    @error('nama_siswa')
                ...
                <div>
                    ...
                    <input type="text" class="mt-1 block w-full h-10 bg-gray-200 rounded-md shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm @error('nisn_id') border-red-500 @enderror" id="nisn_id" name="nisn_id" value="{{ old('nisn_id', $data_siswa->nisn_id) }}" style="padding-left: 10px;">
                    @error('nisn_id')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
