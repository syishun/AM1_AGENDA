        ...
        <div class="w-full md:w-1/2 mb-4 md:mb-0">
            <form action="{{ url('mapel') }}" method="post" enctype="multipart/form-data" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" class="mt-1 block w-full h-10 bg-gray-200 rounded-md shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm @error('nama_mapel') border-red-500 @enderror" id="nama_mapel" name="nama_mapel" value="{{ old('nama_mapel') }}" style="padding-left: 10px;">
                    @error('nama_mapel')
                ...
                <div>
                    ...
                    <input type="text" class="mt-1 block w-full h-10 bg-gray-200 rounded-md shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm @error('mapel_id') border-red-500 @enderror" id="mapel_id" name="mapel_id" value="{{ old('mapel_id') }}" style="padding-left: 10px;">
                    @error('mapel_id')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
