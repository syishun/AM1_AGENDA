        ...
        <div class="w-full md:w-1/2 mb-4 md:mb-0">
            <form action="{{ url('mapel/' . $mapel->id) }}" method="post" enctype="multipart/form-data" class="space-y-4">
                ...
                <div>
                    ...
                    <select class="mt-1 block w-full h-10 bg-gray-200 rounded-md shadow-smfocus:outline-none focus:outline-none focus:ring-2 focus:ring-green-500 sm:text-sm @error('kode_guru') border-red-500 @enderror" name="kode_guru" id="kode_guru" style="padding-left: 10px;">
                ...
                <div>
                    ...
                    <input type="text" class="mt-1 block w-full h-10 bg-gray-200 rounded-md shadow-smfocus:outline-none focus:outline-none focus:ring-2 focus:ring-green-500 sm:text-sm @error('mapel_id') border-red-500 @enderror" id="mapel_id" name="mapel_id" value="{{ old('mapel_id', $mapel->mapel_id) }}" style="padding-left: 10px;">
                    @error('mapel_id')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
